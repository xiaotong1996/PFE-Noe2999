...
public class MyNotifications : MonoBehaviour {
    ...
    public Texture notificationTextureBg;
    ...
    private static float mergin = 15f;

    void Awake(){
        startinPos.x = Screen.width * 0.65f;
        endPos.x = Screen.width * 0.65f;
		...
		textureBg = notificationTextureBg;
		textStyle = myStyle;
    }
	private  static bool pushNotification(string _message,float _duration){
		if(timer <= 0){
            ...
            GUI.DrawTexture(new Rect(currentPos.x- mergin, currentPos.y- mergin, notificationSize.x+ mergin*2, notificationSize.y+ mergin*2), textureBg);
